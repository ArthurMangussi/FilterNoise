...
from Algoritmos.vae_bridge import ConfigVAE, VariationalAutoEncoder
...
class VAEBRIDGE(BaseEstimator):
    ...
    def transform(self, X, y=None):
        ...
        X_wout_mf = np.delete(X, self._missing_feature_idx, axis=1) # deleta a coluna do missing
        if np.isnan(X_wout_mf).astype(int).sum() > 0: # condição para univariado
